...
| **Gate 5: Jev Query Triage**         | OpenRouter Decisions API using `typesafe/jev-1.13`; Noul probability threshold `0.80`.                                                  | Routes benign out-of-scope queries without a strike; malicious queries remain penalized. Jev outage falls back to the local result. | Scope response, strike SSE for malicious activity, or continue to cache on approval. |
