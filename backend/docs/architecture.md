...
| **Gate 5: Jev Query Triage**         | OpenRouter Decisions API using `typesafe/jev-1.13`; Noul probability threshold `0.80`.                                        | Rejects mixed or unrelated intent with a strike. Jev outage falls back to the local result. | Strike SSE response, or continue to cache on approval.                       |
