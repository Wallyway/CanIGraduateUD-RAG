...
_Note: Ignored by frontend JSON parsers according to the W3C SSE standard._
...
## 3. Security Guardrails and Jev Triage Pipeline

CanIGraduateUD-RAG uses a two-stage pre-flight architecture. Local checks for bans, rate limits, prompt injection, explicit abuse, greetings, and basic academic context run without external model tokens. Queries that pass those checks are evaluated by Jev before cache, vector retrieval, or answer generation, protecting the RAG pipeline from mixed or unrelated requests.
...
| **Gate 3: Cyberattack & Abuse Gate** | High-performance regex patterns for prompt injection, jailbreaks (DAN), base64 payloads, malware/tool exploitation, and explicit abuse. | Records strike only for malicious behavior. Benign off-topic and other-faculty queries receive scope guidance without penalty.                                   | Strike warning (Strikes 1-2: HTTP 200 SSE; Strike 3: HTTP 403 SSE 24h ban).                              |
...
| **Gate 5: Jev Query Triage**         | OpenRouter Decisions API using `typesafe/jev-1.13`; Noul probability threshold `0.80`.                                                  | Routes other-faculty queries as interest without a strike; penalizes mixed academic-plus-unrelated intent as evasion. Jev outage falls back to the local result. | Interest/scope response, strike SSE for evasion or malicious activity, or continue to cache on approval. |
| **Gate 6: Hybrid Cache Check**       | Exact SHA-256 hash match (Layer 1) and cosine similarity >= 0.95 (Layer 2).                                                             | Streams cached answer and verified citations immediately.                                                                                                        | HTTP 200 OK SSE with header `X-Cache-Hit: true`.                                                         |

Jev receives the complete query and recent conversation history through the existing OpenRouter API key. Its typed question asks whether the complete request belongs to graduation or academic life for Ingeniería de Sistemas and does not contain a second unrelated task. A result below `0.80` on a query that already passed the academic guardrail is treated as mixed-intent evasion and uses `record_strike`; a clearly identified other-faculty query is routed to interest without penalty. Three strikes cover prompt injection, malware/tool exploitation, explicit abuse, and repeated mixed-intent evasion. Jev never generates user-facing prose.

If Jev is unavailable, returns malformed output, or has no configured key, the backend continues with the local guardrail decision already produced by `inspect_query_safety`. This preserves availability but means novel mixed intents cannot be classified by Jev during that outage.
...
stateDiagram-v2
    [*] --> Clear: New Client
    Clear --> Strike1: 1st Malicious Query
    Strike1 --> Strike2: 2nd Malicious Query
    Strike2 --> Banned24h: 3rd Malicious Query (Max Strikes Reached)
