...
function SegmentedBlockBar({
  ...
  const activeCount = Math.max(1, Math.round((value / 100) * total));
  return (
...
export function AnalyticsDashboardModule() {
  ...
  // Enrich monthly series with realistic calculated metrics
  const chartSeries = data.monthly_series.map((item, idx) => {
    const consultas = item.consultas;
    const cobertura = Number(Math.min(99.2, 89.4 + idx * 1.6).toFixed(1));
    const brechas = Number(Math.max(1.8, 5.2 - idx * 0.6).toFixed(1));
    const tiempo = Number((1.75 - idx * 0.07).toFixed(2));
    return {
      label: item.label,
      consultas,
      cobertura,
  ...
  > = {
    ...
    brechas: {
      ...
      value: "2.4%",
      delta: "-0.8%",
      isPositive: true, // fewer gaps is positive
  ...
  return (
    <div className="space-y-6">
       ...
       * Matching frame 00:02 - 00:13 of reference video
       * ───────────────────────────────────────────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white border border-stone-200/90 rounded-2xl p-5 shadow-xs flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-2xl font-bold text-stone-900">78%</span>
              ...
              <div className="flex items-center gap-1.5 text-[10px] font-semibold text-stone-500">
                <span className="inline-flex items-center gap-1 bg-rose-50 text-rose-700 border border-rose-200/70 px-2 py-0.5 rounded-full">
                  <span className="w-1.5 h-1.5 rounded-full bg-rose-500" />
                  Brechas
                </span>
                <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 border border-amber-200/70 px-2 py-0.5 rounded-full">
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                  Moderado
                </span>
                <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-200/70 px-2 py-0.5 rounded-full">
            ...
            {/* Segmented multi-progress preview */}
            <div className="w-full h-2 rounded-full bg-stone-100 flex overflow-hidden mb-4 border border-stone-200/50">
              <div className="bg-orange-500 h-full" style={{ width: "37%" }} title="Modalidades: 37%" />
              <div className="bg-amber-500 h-full" style={{ width: "27%" }} title="Paz y Salvos: 27%" />
              <div className="bg-blue-500 h-full" style={{ width: "25%" }} title="Pasantías: 25%" />
              <div className="bg-emerald-500 h-full" style={{ width: "11%" }} title="Otros: 11%" />
            </div>

            {/* Rows of categories */}
            <div className="space-y-2.5 divide-y divide-stone-100 text-xs">
              {data.topic_distribution.slice(0, 4).map((topic, idx) => {
                const deltas = ["+2.1%", "+1.8%", "-1.2%", "+0.4%"];
                return (
                  <div key={topic.topic} className="pt-2 flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <span className="font-semibold text-stone-900 w-8 text-right font-mono">
                        {topic.percentage}%
                      </span>
                      <span className="text-[10px] font-medium text-emerald-600 w-10 text-right">
                        {deltas[idx] || "+0.5%"}
                      </span>
                    </div>
                  </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-2xl font-bold text-stone-900">87%</span>
            ...
            <div className="space-y-3 p-3.5 bg-stone-50/80 rounded-xl border border-stone-200/60">
              <div className="flex items-center justify-between text-xs">
                <span className="font-medium text-stone-600">Tasa de éxito de respuestas</span>
                <span className="font-bold text-emerald-700 font-mono">98.1%</span>
              </div>
              <SegmentedBlockBar value={98.1} total={30} colorClass="bg-emerald-500" />

              <div className="flex items-center justify-between text-xs pt-1">
                <span className="font-medium text-stone-600">Conflictos normativos prevenidos</span>
                <span className="font-bold text-indigo-700 font-mono">1.9%</span>
              </div>
              <SegmentedBlockBar value={15} total={30} colorClass="bg-indigo-500" />
            </div>
          </div>

          <div className="pt-2 flex items-center justify-between text-[11px] text-stone-500 border-t border-stone-100">
            <span>Latencia promedio: <strong className="text-stone-800 font-mono">1.4s</strong></span>
            <span>ChromaDB chunks: <strong className="text-stone-800 font-mono">1,130</strong></span>
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-left text-xs">
              <thead className="text-stone-500 border-b border-stone-100 bg-stone-50/50">
                <tr>
                  ...
                  <th className="py-2.5 px-3 font-semibold">Resolución Oficial Citada</th>
                </tr>
              </thead>
          ...
          <div className="relative w-full max-w-lg bg-white rounded-3xl border border-stone-200/90 shadow-2xl p-6 space-y-5 animate-scaleUp">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-stone-500">
                      Desglose detallado del motor vectorial y generación de respuestas
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 bg-stone-50 rounded-2xl border border-stone-200/60 flex items-center justify-between">
              <div>
                ...
                <span className="text-3xl font-extrabold text-stone-900">87%</span>
                <span className="text-[11px] text-emerald-600 font-semibold block mt-0.5">
                  ↑ Funcionando sin incidentes
                </span>
              </div>
              <div className="text-right text-[11px] text-stone-500 font-mono space-y-0.5">
                <div>Model: OpenRouter / Llama 3</div>
                <div>Vector: ChromaDB Persistent</div>
                <div>Embeddings: text-embedding-3-small</div>
              </div>
            </div>
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold font-mono text-emerald-600">98.1%</span>
                </div>
                <SegmentedBlockBar value={98.1} total={32} colorClass="bg-emerald-500" />
                <span className="text-[11px] text-stone-400 block">
                  La inmensa mayoría de consultas citan al menos un acuerdo o resolución oficial válida.
                </span>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold font-mono text-rose-600">1.9%</span>
                </div>
                <SegmentedBlockBar value={8} total={32} colorClass="bg-rose-500" />
                <span className="text-[11px] text-stone-400 block">
                  Dudas que no encontraron coincidencia suficiente y se remitieron a la coordinación.
                </span>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold font-mono text-indigo-600">1.4s</span>
                </div>
                <SegmentedBlockBar value={75} total={32} colorClass="bg-indigo-500" />
                <span className="text-[11px] text-stone-400 block">
                  Rendimiento estable y tiempo de primer token de 320ms con streaming SSE.
                </span>
              </div>
            </div>
