...
export interface ThinkingStateProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  workingLabel?: string;
  durationSeconds?: number;
  settledLabel?: (seconds: number) => React.ReactNode;
  onSettled?: () => void;
}
...
export const ThinkingState = React.forwardRef<HTMLDivElement, ThinkingStateProps>(
  (
    ...
    const [internalActiveIndex, setInternalActiveIndex] = React.useState(autoPlay ? 0 : totalNodes);
    const [internalIsWorking, setInternalIsWorking] = React.useState(autoPlay);
    const [manualExpanded, setManualExpanded] = React.useState<boolean | null>(
      defaultExpanded !== undefined ? defaultExpanded : null
    );

    const isWorking = isWorkingProp !== undefined ? isWorkingProp : internalIsWorking;
    const activeIndex = isWorking
      ? Math.min(internalActiveIndex, Math.max(0, totalNodes - 1))
      : totalNodes;

    const startTimeRef = React.useRef<number>(Date.now());
    const [elapsedSeconds, setElapsedSeconds] = React.useState<number>(durationSeconds ?? 0);
    ...
    const advanceStep = React.useCallback(() => {
      setInternalActiveIndex((prev) => {
        const next = prev + 1;
        if (next >= totalNodes) {
          if (isWorkingProp === undefined) {
            setInternalIsWorking(false);
          }
    ...
    }, [totalNodes, isWorkingProp]);

    React.useEffect(() => {
      if (!autoPlay || !isWorking || internalActiveIndex >= totalNodes) return;

      const currentNode = nodes[internalActiveIndex];
    ...
    }, [internalActiveIndex, autoPlay, isWorking, totalNodes, nodes, advanceStep]);
